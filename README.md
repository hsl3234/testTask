...
### Дополнительно к ТЗ (осознанно, для демонстрации «production»-подхода)

Ниже — то, чего **не требовало** исходное ТЗ, но что добавлено **намеренно**, чтобы показать привычный для продакшена контур: воспроизводимая схема БД, договор API, сопровождаемость и сценарий админки с сессиями. Это не «раздувание» объёма, а явные инженерные допущения.

| Дополнение | Зачем |
| ---------- | ----- |
| **Phalcon Migrations** (`phalcon-migrations`, версии `1.0.0` / `1.0.1`) | Схема и сиды в коде, накат при старте Docker / через `composer run migrate` — без ручного `init.sql` и с понятной историей изменений. |
| **Публичный контракт API в camelCase** + приём **snake_case** в query/body | Согласованность с фронтом (Vue) и OpenAPI; алиасы не ломают старые примеры с `per_page` / `category_id`. |
| **Полноценный вход в админку** — `POST /api/auth/login`, `POST /api/auth/refresh`, short-lived access + одноразовый refresh, экран логина, **refresh-on-401** в SPA, сид админа из `.env` | ТЗ требовало лишь носитель **Bearer**; login/password и ротация — типичный для UI способ выдачи токенов. Статический `demo-token-please-change` оставлен **намеренно** для curl и Swagger. |
| **OpenAPI 3** (zircote/swagger-php) и **Swagger UI** на `/api/docs` | Живая спецификация рядом с кодом; удобнее, чем только статичные примеры в README. |
| **GET /api/health** | Liveness / проверка стека без авторизации (удобно для оркестрации и healthcheck). |
| **Docker Compose** (PHP-FPM, Nginx, MySQL) | Соответствует форме сдачи (локально или Docker) — один `docker compose up` для ревью. |
...
| ---------- | ------------------ |
| **Главная страница — админка (Volt + Vue): вход, товары, категории** | **http://localhost:8080/** |
